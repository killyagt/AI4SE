...
- [x] T6：先运行 `python -m pip install -e ".[dev]"`，再运行 `pytest -q`；构建 wheel、配置 CI、创建远程仓库和 Release。最终测试 `11 passed`；wheel 为 `safe_coding_agent_harness-0.1.0-py3-none-any.whl`；GitHub Actions 最近一次运行成功；Release 为 `v0.1.0`。
